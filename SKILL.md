...
4. **输出判定** — 骗局类型（对照 references/scam-knowledge.md；未收录时输出"未收录类型，按特征判定"，用通用特征速记"五要"信号逐条说明命中/未命中，定级依据引用 references/risk-levels.md 谓词，结论标 [推断]，禁止编造类型名）、证据拆解（为什么像/不像）、按人群适配的应对话术（references/response-scripts.md）
